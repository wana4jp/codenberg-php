...
class CustomField implements ModelInterface, ArrayAccess
{
    ...
    {
        $allowedValues = $this->getFieldTypeAllowableValues();
        if (!is_null($fieldType) && !in_array($fieldType, $allowedValues)) {
    ...
    {
        $allowedValues = $this->getTextAlignAllowableValues();
        if (!is_null($textAlign) && !in_array($textAlign, $allowedValues)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value for 'textAlign', must be one of '%s'",
                    implode("', '", $allowedValues)
    ...
    {
        $allowedValues = $this->getWritingModeAllowableValues();
        if (!is_null($writingMode) && !in_array($writingMode, $allowedValues)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value for 'writingMode', must be one of '%s'",
                    implode("', '", $allowedValues)
